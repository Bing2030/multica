...
To avoid waiting for cron while debugging an autopilot, trigger it manually — click **Run now** on the autopilot detail page.
...
- **Rotate it if it leaks** — click **Rotate URL** on the trigger row in the autopilot detail page. The old URL stops working immediately.
