...
Multica stores user-uploaded attachments (images and files in comments) on the **local filesystem** of the server host. There is no object-store backend: `S3_*`, `AWS_*`, `CLOUDFRONT_*`, and `ATTACHMENT_DOWNLOAD_*` variables from older versions are no longer read and have no effect.

### Local disk

| Variable | Default | Description |
|---|---|---|
| `LOCAL_UPLOAD_DIR` | `./data/uploads` | Directory where uploaded files are written. The server creates it at startup if it does not exist |
| `LOCAL_UPLOAD_BASE_URL` | empty | Optional public base URL that serves `LOCAL_UPLOAD_DIR` — for example `http://localhost:8080` when the same origin serves `/uploads/*`, or the URL of a reverse proxy that serves the directory. When set, attachment content URLs are emitted as absolute URLs against this base; when unset, they are site-relative (`/uploads/...`) |

### How attachment URLs resolve

Uploads are served from `/uploads/<key>` under the server origin (or under `LOCAL_UPLOAD_BASE_URL` when configured). Every attachment also has a stable, server-proxied download endpoint — `GET /api/attachments/{id}/download` — which streams the file on each request with no signed URL and no TTL. API `download_url` values use this endpoint, anchored on `MULTICA_PUBLIC_URL` so they resolve for clients that don't share the document origin (Desktop, mobile webview).

URLs persisted into markdown bodies follow the same rule: when `LOCAL_UPLOAD_BASE_URL` is configured (uploads served from a known public host) and the stored URL is an absolute, unsigned `http(s)` URL, that URL is persisted directly; otherwise the server-proxied `/api/attachments/{id}/download` path is used.
...
Product analytics are captured **client-side** by posthog-js in the browser / Electron client. The server does not capture events itself — it only exposes the PostHog key and host to clients via `/api/config`. Leave `POSTHOG_API_KEY` empty for local dev or self-hosted instances and no client-side capture runs.

| Variable | Default | Description |
|---|---|---|
| `POSTHOG_API_KEY` | empty | PostHog project key the client uses to initialize posthog-js. Empty disables client-side capture |
| `POSTHOG_HOST` | `https://us.i.posthog.com` | PostHog host the client reports to. Change to your own host if you self-host PostHog |
| `ANALYTICS_ENVIRONMENT` | derived from `APP_ENV` | Override for the `environment` PostHog event property (normalizes to `production` / `staging` / `dev`) |
| `ANALYTICS_DISABLED` | `false` | Set `true` to suppress client-side capture even when `POSTHOG_API_KEY` is set (CI / opt-out) |
