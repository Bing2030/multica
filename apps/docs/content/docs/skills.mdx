...
After attaching, the agent picks up its skills the next time it starts a task — each AI coding tool has its own skill discovery path (Claude Code uses `.claude/skills/`, Cursor uses `.cursor/skills/`, Codex uses `$CODEX_HOME/skills/`, etc.), and Multica drops files in the right place automatically. The full per-tool path mapping is in [AI coding tools comparison → Where skill files go](/providers#where-skill-files-go).
...
The two are complementary. In Multica today, MCP support is **provider-specific**: Claude Code, Codex, Cursor, and OpenCode consume `mcp_config`; the other supported tools (Copilot and Pi) receive the field but don't actually use it. A dedicated MCP section will come in a later release.
...
- [AI coding tools comparison](/providers) — full comparison of all 6 tools (including each one's skill injection path)
