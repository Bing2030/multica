...
In day-to-day use you'll only touch the first two directly. The **[daemon](/daemon-runtimes) token** is created and refreshed automatically by the daemon when it starts with a valid PAT in `~/.multica/config.json` — you don't have to think about it.
...
**Creating** a PAT is done in the web UI: **Settings → Personal Access Tokens → New token**. Once created, paste it into the `token` field of `~/.multica/config.json` so the daemon can authenticate with it.
...
When you click log out in the Web UI, the browser deletes the JWT cookie. To remove a daemon's credentials, delete `~/.multica/config.json` (or just the `token` field) and stop the daemon with `multica daemon stop`.

Either way:

- **The local token is cleared** — the browser deletes the cookie; the daemon can no longer authenticate.
- **The PAT is still valid on the server** — if someone obtained your PAT before you removed it (for example, by copying it to another machine), they **can still use it**.
...
- [Daemon commands](/cli) — the remaining `multica` binary commands
