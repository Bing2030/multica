...
A **manual rerun** is one you trigger from the web UI (the retry button on an execution-log row) or directly from the API (`POST /api/issues/{id}/rerun`).
...
| Agent picked | Same agent as the failed task | Source task's agent (per-row retry) or issue's current assignee (issue-level rerun) |
...
All 6 supported tools implement session resumption, though the mechanics differ per tool (Pi, for example, uses a session file path as its resume id rather than a string id):

- ✅ **Real support** — Claude Code, Codex, Copilot, Cursor, OpenCode, Pi

See [Providers Matrix → Session resumption](/providers#session-resumption-who-really-supports-it).

## Next

- [Providers Matrix](/providers) — capability differences across the 6 AI coding tools (including the exact session-resumption status)
