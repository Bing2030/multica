...
- **Local [daemon](/daemon-runtimes)** — you run `multica daemon` on your own machine, and it drives the [AI coding tools](/providers) installed locally. Six are built in today: [Claude Code](/providers#claude-code), [Codex](/providers#codex), [Cursor](/providers#cursor), [Copilot](/providers#copilot), [OpenCode](/providers#opencode), [Pi](/providers#pi). Your API keys, toolchain, and code directories stay on your machine.
...
<NumberedCards>
  <NumberedCard number="01" title="Multica Cloud" href="/cloud-quickstart" tag="Waitlist">
    Managed backend. Install the daemon (or the desktop app), and connect to the Multica-hosted server. Takes about 5 minutes.
  </NumberedCard>
