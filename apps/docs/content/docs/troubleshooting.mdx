...
3. **Token expired or invalid** — the PAT in `~/.multica/config.json` is missing or was revoked
...
cat ~/.multica/config.json        # verify the token field exists
```

In the web UI, confirm you're a member of the target workspace by opening the workspace switcher.

**How to fix**: address each cause above. The two most common fixes are **changing `MULTICA_SERVER_URL` (or the `server_url` field in `~/.multica/config.json`) and restarting the daemon** (`multica daemon restart`) and **generating a fresh PAT** — create a new one under **Settings → Personal Access Tokens** in the web app, paste it into `~/.multica/config.json`'s `token` field, then restart the daemon.
...
In the web UI, open **Settings → Agents** to check each agent's archived state, and open the issue's **Activity** or **Execution log** tab to inspect task history.

On the server side (self-host), grep for `"no_tasks"` / `"no_capacity"` to see the claim outcome.

**How to fix**:

- Concurrency full → wait for running tasks to finish, or raise the agent's concurrency limit in **Settings → Agents** (edit the agent and increase `max_concurrent_tasks`)
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → restore it from **Settings → Agents** (open the archived agent and click **Restore**)
...
**Symptom**: `multica daemon start` fails with `address already in use`.
