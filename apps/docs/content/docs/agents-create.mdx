...
Once that's in place, go to the **Agents** page in your workspace and click **+ New**.
...
Each runtime is backed by a specific AI coding tool. Multica supports 6 of them:
...
| **OpenCode** | Open source (SST); bring your own model catalog |
| **Pi** | Minimalist, from Inflection AI |

Each tool's full capability matrix (session resume, MCP, skill injection path, model selection) is covered in [AI coding tools comparison](/providers).
...
- [AI coding tools comparison](/providers) — full capability matrix across all 6 tools
