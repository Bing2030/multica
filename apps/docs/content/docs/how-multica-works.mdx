...
- **AI coding tools** — one of the six (or several in parallel): [Claude Code](/providers#claude-code), [Codex](/providers#codex), [Cursor](/providers#cursor), [Copilot](/providers#copilot), [OpenCode](/providers#opencode), [Pi](/providers#pi). Once the daemon has picked up a task, it uses these tools to actually do the work.
