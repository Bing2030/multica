...
## 5. Point the daemon at your own server

The daemon reads `~/.multica/config.json` at start. You create this file by hand — there is no `multica setup` to do it for you. The exact field values depend on where the daemon runs relative to the server.

### 5a. Same machine

If the daemon and the server run on the same host, the loopback defaults work. Create `~/.multica/config.json`:
...
Create the PAT (`mul_…`) in the web app under **Settings → Personal Access Tokens → New token**. Then start the daemon:
...
Because the compose stack only listens on `127.0.0.1`, a daemon on a different machine cannot reach `http://<server-ip>:8080` directly — and you do not want it to, since server secrets would otherwise be reachable from the open internet. Put a reverse proxy on the server that terminates TLS and forwards to `127.0.0.1:8080` (backend) and `127.0.0.1:3000` (frontend), then point `~/.multica/config.json` at the public HTTPS URL:

```json
{
  "server_url": "wss://multica.example.com/ws",
...
<Callout type="info">
Prefer environment variables over editing the file? The daemon also reads `MULTICA_SERVER_URL` and `MULTICA_APP_URL` from the environment; a value in `config.json` takes precedence, but the env vars are handy for one-off overrides. `MULTICA_SERVER_URL` also accepts the `ws://…/ws` daemon form from [Environment variables](/environment-variables) and normalizes it to the HTTP base.
</Callout>
...
Then open `http://multica.dev.lan` and continue at [Step 4 — First login](#4-first-login--create-a-workspace) above. Point the daemon at your Ingress hostnames by creating `~/.multica/config.json`:

```json
{
  "server_url": "ws://api.multica.dev.lan/ws",
  "app_url": "http://multica.dev.lan",
  "token": "mul_..."
}
```

Then start the daemon with `multica daemon start`.
