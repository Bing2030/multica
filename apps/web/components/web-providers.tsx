...
export function WebProviders({
  ...
  return (
    <CoreProvider
      ...
      onLogout={() => {
        // THROWAWAY POC: under DevBypass there is no real logout (the next
        // request re-stamps the dev user); we only clear the local logged-in
        // cookie so any browser-level gating stays consistent. NEVER MERGE.
        clearLoggedInCookie();
      }}
      identity={identity}
      locale={locale}
