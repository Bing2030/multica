...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, settings_path
`
...
const clearAgentSettingsPath = `-- name: ClearAgentSettingsPath :one
...
func (q *Queries) ClearAgentSettingsPath(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentSettingsPath, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
type CreateAgentParams struct {
	...
	SettingsPath       pgtype.Text `json:"settings_path"`
}
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, settings_path FROM agent
WHERE id = $1
`
...
UPDATE agent SET
    ...
    settings_path = COALESCE($17, settings_path),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, settings_path
`
