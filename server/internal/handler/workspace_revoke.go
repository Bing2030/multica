...
// per-task status poller interrupts the running agent gracefully), and finally
// the member row itself is removed.
//
...
// regardless of how the daemon authenticates. The agent-archive + task-cancel
// + force-offline writes are the production safety net: even if a daemon
// races back online, it finds no agent it can run for, no queued task to
// claim, and the dispatcher (which gates on agent.archived_at IS NULL) won't
// hand it new work — and the member-row deletion in the same tx means
// subsequent requireWorkspaceMember checks reject its requests with 404.
//
// THROWAWAY POC: the daemon_token revocation step (deleting mdt_ rows +
// invalidating the daemon-token cache) is gone with the rest of the login
// surface — DevBypass is the sole identity path. NEVER MERGE.
//
...
// publishRevocation runs all post-commit side effects: broadcast
// task:cancelled with per-agent reconciliation, broadcast agent:archived, and
// signal a runtime-list refresh. Safe to call on an empty result — it returns
// immediately.
