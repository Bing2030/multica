...
type PrepareParams struct {
	...
	// via a per-task config file. Cursor consumes it here; other providers
	// wire MCP via ExecOptions.McpConfig in the agent backend.
...
// the per-provider knobs (CodexVersion) so callers can pass the same
// resolved binary path on both first-run and reuse paths.
