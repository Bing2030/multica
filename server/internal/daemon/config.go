...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, codex, copilot, opencode, pi, cursor
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, pi, or cursor-agent and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	"claude", "codex", "opencode", "pi", "cursor-agent", "copilot",
}
